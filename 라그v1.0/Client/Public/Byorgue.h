...
class CByorgue final : public CGameObject
{
public:
	enum STATE { IDLE, DEAD, SKILL, MOVE, STATE_END };
private:
	CByorgue(LPDIRECT3DDEVICE9 _pGraphic_Device);
	CByorgue(const CByorgue& rhs);
	virtual ~CByorgue() = default;
...
public:
	static CByorgue* Create(LPDIRECT3DDEVICE9 _pGraphic_Device);
