...
CAtros::CAtros(LPDIRECT3DDEVICE9 _pGraphic_Device)
	: CGameObject(_pGraphic_Device)
{
}

CAtros::CAtros(const CAtros& rhs)
	: CGameObject(rhs)
{
}
HRESULT CAtros::Initialize_Prototype(void)
{
...
HRESULT CAtros::Initialize(void * pArg)
{
	...
	m_tInfo.iDmg = 66;
	m_tInfo.fX = 0.5f;
	m_tInfo.iMaxHp = 25000;
	m_tInfo.iHp = m_tInfo.iMaxHp;
	m_tInfo.iMp = 1;
	m_tInfo.iExp = 20;
	m_tInfo.iMonsterType = (_int)MON_ATROS;
	...
	tInfo.iMonsterType = (_int)MON_ATROS;
...
void CAtros::Tick(_float fTimeDelta)
{
	...
	if (!m_bRespawn)
	{
		...
		{
			ERR_MSG(TEXT("Failed to Add CollisionGroup : CAtros"));
			return;
		}
...
void CAtros::Late_Tick(_float fTimeDelta)
{
...
HRESULT CAtros::SetUp_Components(void)
{
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_IDLE_Front"), (CComponent**)&m_pTextureComIDLE_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_IDLE_Back"), (CComponent**)&m_pTextureComIDLE_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_Move_Front"), (CComponent**)&m_pTextureComMove_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_Move_Back"), (CComponent**)&m_pTextureComMove_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_Attack_Front"), (CComponent**)&m_pTextureComAttack_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_Attack_Back"), (CComponent**)&m_pTextureComAttack_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_Dead_Front"), (CComponent**)&m_pTextureComDead_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Atros_Dead_Back"), (CComponent**)&m_pTextureComDead_Back)))
		return E_FAIL;
...
HRESULT CAtros::SetUp_RenderState(void)
{
...
HRESULT CAtros::Release_RenderState(void)
{
...
void CAtros::Chase(_float fTimeDelta)
{
...
void CAtros::Chase2(_float fTimeDelta)
{
...
void CAtros::Chase3(_float fTimeDelta)
{
	_float Distance = D3DXVec3Length(&(*(_float3*)&m_tInfo.pTarget->Get_World().m[3][0] - m_pTransformCom->Get_State(CTransform::STATE_POSITION)));
	if (1.f >= Distance)
	{
...
CAtros * CAtros::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CAtros* pInstance = new CAtros(_pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CAtros"));
		Safe_Release(pInstance);
	}
...
CGameObject * CAtros::Clone(void * pArg)
{
	CAtros* pInstance = new CAtros(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CAtros"));
		Safe_Release(pInstance);
	}
...
_float4x4 CAtros::Get_World(void)
{
	return m_pTransformCom->Get_WorldMatrix();
}

void CAtros::Free(void)
{
	__super::Free();
...
HRESULT CAtros::On_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CAtros::Off_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CAtros::Skill_DefaultAttack(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_SwordSlice"), m_tInfo.iLevelIndex, pLayerTag, &tInfo)))
		return E_FAIL;
	CSoundMgr::Get_Instance()->PlayEffect(L"Atros_Attack.wav", fSOUND);
	Safe_Release(pGameInstance);

	return S_OK;
}
void CAtros::Motion_Change()
{
...
void CAtros::Move_Frame(_float fTimeDelta)
{
	switch (m_eCurState)
	{
	case IDLE:
...
void CAtros::Check_Front()
{
	...
	if (m_tInfo.bDead && m_eCurState != DEAD)
	{
		...
		CSoundMgr::Get_Instance()->PlayEffect(L"Atros_Die.wav", fSOUND);
	}
...
void CAtros::Use_Skill(_float fTimeDelta)
{
...
void CAtros::MonsterMove(_float fTimeDelta)
{
