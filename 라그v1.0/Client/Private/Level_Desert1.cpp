#include "stdafx.h"
#include "..\Public\Level_Desert1.h"
...
CLevel_Desert1::CLevel_Desert1(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CLevel(pGraphic_Device)
{
	ZeroMemory(&Info, sizeof(CGameObject::INFO));
}

HRESULT CLevel_Desert1::Initialize()
{
...
void CLevel_Desert1::Tick(_float fTimeDelta)
{
...
void CLevel_Desert1::Late_Tick(_float fTimeDelta)
{
	__super::Late_Tick(fTimeDelta);

	SetWindowText(g_hWnd, TEXT("사막레벨입니다."));

	Open_Level();
}

HRESULT CLevel_Desert1::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	ZeroMemory(&info, sizeof(CGameObject::INFO));
	info.pstrPath = TEXT("../../Data/Terrain/Town2Hg1.dat");
	info.iLevelIndex = LEVEL_DESERT1;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Terrain"), LEVEL_DESERT1, pLayerTag, &info)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Sky"), LEVEL_DESERT1, pLayerTag, &info)))
		return E_FAIL;
	...
	for (auto& iter : m_vecHouse)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House"), LEVEL_DESERT1, pLayerTag, &indexpos)))
			return E_FAIL;
	}
	...
	for (auto& iter : m_vecHouse2)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House2"), LEVEL_DESERT1, pLayerTag, &indexpos)))
			return E_FAIL;
	}
	...
	for (auto& iter : m_vecIndex)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BackGroundRect"), LEVEL_DESERT1, pLayerTag, &indexpos)))
			return E_FAIL;
	}
	...
	for (auto& iter : m_vecWall)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Transparent_Wall"), LEVEL_DESERT1, pLayerTag, &Wall)))
			return E_FAIL;
	}
	...
	//if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House3"), LEVEL_DESERT1, pLayerTag, &index)))
	//	return E_FAIL;

	//if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House4"), LEVEL_DESERT1, pLayerTag, nullptr)))
	//	return E_FAIL;

	//if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House5"), LEVEL_DESERT1, pLayerTag, nullptr)))
	//	return E_FAIL;
	...
	//if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House6"), LEVEL_DESERT1, pLayerTag, &index2)))
	//	return E_FAIL;
	...
	//if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House6"), LEVEL_DESERT1, pLayerTag, &index3)))
	//	return E_FAIL;


	//if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House7"), LEVEL_DESERT1, pLayerTag, nullptr)))
	//	return E_FAIL;


	//if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_ItemBox"), LEVEL_DESERT1, pLayerTag, nullptr)))
	//	return E_FAIL;
...
HRESULT CLevel_Desert1::Ready_Layer_Player(const _tchar * pLayerTag)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (pGameInstance->Find_Layer(LEVEL_STATIC, TEXT("Layer_PlayerInfo")) != nullptr)
	{
		...
		if (Info.iLevelIndex == LEVEL_TOWN2)
		{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Player"), LEVEL_DESERT1, pLayerTag, &Info)))
			return E_FAIL;
	}
	else
	{
...
HRESULT CLevel_Desert1::Ready_Layer_Monster(const _tchar * pLayerTag)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	for (; iter != m_vMonsterPos1.end(); ++iter)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Atros"), LEVEL_DESERT1, pLayerTag, &Info)))
			return E_FAIL;
	...
	for (; iter != m_vMonsterPos1.end(); ++iter)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Baphomet"), LEVEL_DESERT1, pLayerTag, &Info)))
			return E_FAIL;
	...
	for (; iter != m_vMonsterPos1.end(); ++iter)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Minorous"), LEVEL_DESERT1, pLayerTag, &Info)))
			return E_FAIL;
...
HRESULT CLevel_Desert1::Ready_Layer_Camera(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Camera_Dynamic"), LEVEL_DESERT1, pLayerTag, &CameraDesc)))
		return E_FAIL;
...
HRESULT CLevel_Desert1::Ready_Layer_UI(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_ExpBlank"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_ExpLogo"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_QuickSlot"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_HpPotion"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Inventory"), LEVEL_DESERT1, TEXT("Layer_Inventory"), &tInfo)))
		return E_FAIL;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_IconBar"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_InventoryIcon"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_EquipIcon"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_SkillIcon"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;

	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_StatsIcon"), LEVEL_DESERT1, pLayerTag, &tInfo)))
		return E_FAIL;
...
HRESULT CLevel_Desert1::Ready_Layer_NPC(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	for (auto& iter : m_vecNPC)
	{
		...
		else if (iter.iIndex > 0 && iter.iIndex <= 3)
		{
			...
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Default_NPC"), LEVEL_DESERT1, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 4)
		{
			...
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Engineer"), LEVEL_DESERT1, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 5)
		{
			tInfo.vPos = iter.BackGroundPos;
			tInfo.iMp = iter.iIndex;
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Village_Chief"), LEVEL_DESERT1, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 6)
		{
			tInfo.vPos = iter.BackGroundPos;
			tInfo.iMp = iter.iIndex;
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Village_Quest1"), LEVEL_DESERT1, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 7)
		{
			tInfo.vPos = iter.BackGroundPos;
			tInfo.iMp = iter.iIndex;
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Village_Quest2"), LEVEL_DESERT1, pLayerTag, &tInfo)))
				return E_FAIL;
		}
	}
...
HRESULT CLevel_Desert1::Ready_Layer_Portal(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
void CLevel_Desert1::Open_Level(void)
{
	CGameInstance*		pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);

	if (nullptr != pGameInstance->Find_Layer(LEVEL_DESERT1, TEXT("Layer_Portal")))
	{
		for (auto& iter : pGameInstance->Find_Layer(LEVEL_DESERT1, TEXT("Layer_Portal"))->Get_Objects())
		{
...
void CLevel_Desert1::LoadData()
{
	HANDLE hFile = CreateFile(TEXT("../../Data/Town2HgPos1.dat"), GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
CLevel_Desert1 * CLevel_Desert1::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CLevel_Desert1*	pInstance = new CLevel_Desert1(pGraphic_Device);
