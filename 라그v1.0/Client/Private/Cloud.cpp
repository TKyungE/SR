...
void CCloud::Move_Frame(_float fTimeDelta)
{
	switch (m_eCurState)
	{
	case IDLE:
...
HRESULT CCloud::On_SamplerState()
{
...
HRESULT CCloud::Off_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
CCloud * CCloud::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		ERR_MSG(TEXT("Failed to Created : CCloud"));
...
CGameObject * CCloud::Clone(void* pArg)
{
	...
	{
		ERR_MSG(TEXT("Failed to Cloned : CCloud"));
		Safe_Release(pInstance);
	}
...
_float4x4 CCloud::Get_World(void)
{
