...
HRESULT CLEVEL_GamePlay::Ready_Layer_UI(const _tchar * pLayerTag)
{
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Fade"), LEVEL_GAMEPLAY, pLayerTag, &tInfo)))
		return E_FAIL;
		...
		for (auto& iter : pGameInstance->Find_Layer(LEVEL_GAMEPLAY, TEXT("Layer_Portal"))->Get_Objects())
		{
			if (dynamic_cast<CPortal*>(iter)->Get_Level())
			{
				if (!m_bFade)
				{
					...
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Fade"), LEVEL_GAMEPLAY, TEXT("Layer_Fade"), &tInfo)))
						return;
					m_bFade = true;
				}

				if (pGameInstance->Find_Layer(LEVEL_GAMEPLAY, TEXT("Layer_Fade"))->Get_Objects().front()->Get_Info().iMaxMp == 1)
				{
