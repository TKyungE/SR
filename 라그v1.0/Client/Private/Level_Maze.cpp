...
HRESULT CLevel_Maze::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Sky"), LEVEL_MAZE, pLayerTag, &info)))
		return E_FAIL;
	...
	for (auto& iter : m_vecHouse)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House"), LEVEL_MAZE, pLayerTag, &indexpos)))
			return E_FAIL;
	}
	...
	for (auto& iter : m_vecHouse2)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_House2"), LEVEL_MAZE, pLayerTag, &indexpos)))
			return E_FAIL;
	}
	...
	for (auto& iter : m_vecIndex)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BackGroundRect"), LEVEL_MAZE, pLayerTag, &indexpos)))
			return E_FAIL;
	}
...
HRESULT CLevel_Maze::Ready_Layer_Player(const _tchar * pLayerTag)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (pGameInstance->Find_Layer(LEVEL_STATIC, TEXT("Layer_PlayerInfo")) != nullptr)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Player"), LEVEL_MAZE, pLayerTag, &Info)))
			return E_FAIL;
	}
	else
	{
...
HRESULT CLevel_Maze::Ready_Layer_UI(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_Maze::Ready_Layer_NPC(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	for (auto& iter : m_vecNPC)
	{
		if (iter.iIndex <= 3)
		{
			...
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Default_NPC"), LEVEL_MAZE, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 4)
		{
			...
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Engineer"), LEVEL_MAZE, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 5)
		{
			tInfo.vPos = iter.BackGroundPos;
			tInfo.iMp = iter.iIndex;
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Village_Chief"), LEVEL_MAZE, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 6)
		{
			tInfo.vPos = iter.BackGroundPos;
			tInfo.iMp = iter.iIndex;
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Village_Quest1"), LEVEL_MAZE, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex == 7)
		{
			tInfo.vPos = iter.BackGroundPos;
			tInfo.iMp = iter.iIndex;
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Village_Quest2"), LEVEL_MAZE, pLayerTag, &tInfo)))
				return E_FAIL;
		}
		else if (iter.iIndex > 7)
		{
...
HRESULT CLevel_Maze::Ready_Layer_Portal(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	for (; iter != m_vecPortal.end(); ++iter)
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Portal"), LEVEL_MAZE, pLayerTag, &tInfo)))
			return E_FAIL;
...
void CLevel_Maze::Open_Level(void)
{
	CGameInstance*		pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);

	if (nullptr != pGameInstance->Find_Layer(LEVEL_MAZE, TEXT("Layer_Portal")))
	{
		for (auto& iter : pGameInstance->Find_Layer(LEVEL_MAZE, TEXT("Layer_Portal"))->Get_Objects())
		{
			if (dynamic_cast<CPortal*>(iter)->Get_Level())
			{
				if (!m_bFade)
				{
					...
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Fade"), LEVEL_MAZE, TEXT("Layer_Fade"), &tInfo)))
						return;
					m_bFade = true;
				}

				if (pGameInstance->Find_Layer(LEVEL_MAZE, TEXT("Layer_Fade"))->Get_Objects().front()->Get_Info().iMaxMp == 1)
				{
					pGameInstance->Find_Layer(LEVEL_STATIC, TEXT("Layer_PlayerInfo"))->Get_Objects().front()->Set_Info(pGameInstance->Find_Layer(LEVEL_MAZE, TEXT("Layer_Player"))->Get_Objects().front()->Get_Info());
