...
HRESULT CZombie::Initialize_Prototype(void)
{
...
HRESULT CZombie::SetUp_Components(void)
{
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_IDLE_Front"), (CComponent**)&m_pTextureComIDLE_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_IDLE_Back"), (CComponent**)&m_pTextureComIDLE_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_Move_Front"), (CComponent**)&m_pTextureComMove_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_Move_Back"), (CComponent**)&m_pTextureComMove_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_Attack_Front"), (CComponent**)&m_pTextureComAttack_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_Attack_Back"), (CComponent**)&m_pTextureComAttack_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_Dead_Front"), (CComponent**)&m_pTextureComDead_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Zombie_Dead_Back"), (CComponent**)&m_pTextureComDead_Back)))
		return E_FAIL;
...
HRESULT CZombie::SetUp_RenderState(void)
{
...
HRESULT CZombie::Release_RenderState(void)
{
...
void CZombie::Chase(_float fTimeDelta)
{
...
void CZombie::Chase2(_float fTimeDelta)
{
...
void CZombie::Chase3(_float fTimeDelta)
{
	_float Distance = D3DXVec3Length(&(*(_float3*)&m_tInfo.pTarget->Get_World().m[3][0] - m_pTransformCom->Get_State(CTransform::STATE_POSITION)));
	if (1.f >= Distance)
	{
...
CZombie * CZombie::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CZombie* pInstance = new CZombie(_pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CZombie"));
		Safe_Release(pInstance);
	}
...
CGameObject * CZombie::Clone(void * pArg)
{
	...
	{
		ERR_MSG(TEXT("Failed to Cloned : CZombie"));
		Safe_Release(pInstance);
	}
...
_float4x4 CZombie::Get_World(void)
{
...
HRESULT CZombie::On_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CZombie::Off_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CZombie::Skill_DefaultAttack(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_DefaultAttack"), m_tInfo.iLevelIndex, pLayerTag, &tInfo)))
		return E_FAIL;
	CSoundMgr::Get_Instance()->PlayEffect(L"Zombie_Attack.wav", fSOUND);
	Safe_Release(pGameInstance);
...
void CZombie::Move_Frame(_float fTimeDelta)
{
	switch (m_eCurState)
	{
	case IDLE:
...
void CZombie::Use_Skill(_float fTimeDelta)
{
...
void CZombie::MonsterMove(_float fTimeDelta)
{
