...
CMinorous::CMinorous(LPDIRECT3DDEVICE9 _pGraphic_Device)
	: CGameObject(_pGraphic_Device)
{
}

CMinorous::CMinorous(const CMinorous& rhs)
	: CGameObject(rhs)
{
}
HRESULT CMinorous::Initialize_Prototype(void)
{
...
HRESULT CMinorous::Initialize(void * pArg)
{
	...
	m_tInfo.iDmg = 66;
	m_tInfo.fX = 0.5f;
	m_tInfo.iMaxHp = 40000;
	m_tInfo.iHp = m_tInfo.iMaxHp;
	m_tInfo.iMp = 1;
	m_tInfo.iExp = 40;
	m_tInfo.iMonsterType = (_int)MON_MINOROUS;
...
void CMinorous::Tick(_float fTimeDelta)
{
	...
	if (!m_bRespawn)
	{
		...
		{
			ERR_MSG(TEXT("Failed to Add CollisionGroup : CMinorous"));
			return;
		}
...
void CMinorous::Late_Tick(_float fTimeDelta)
{
...
HRESULT CMinorous::SetUp_Components(void)
{
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_IDLE_Front"), (CComponent**)&m_pTextureComIDLE_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_IDLE_Back"), (CComponent**)&m_pTextureComIDLE_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_Move_Front"), (CComponent**)&m_pTextureComMove_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_Move_Back"), (CComponent**)&m_pTextureComMove_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_Attack_Front"), (CComponent**)&m_pTextureComAttack_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_Attack_Back"), (CComponent**)&m_pTextureComAttack_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_Dead_Front"), (CComponent**)&m_pTextureComDead_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Minorous_Dead_Back"), (CComponent**)&m_pTextureComDead_Back)))
		return E_FAIL;
...
HRESULT CMinorous::SetUp_RenderState(void)
{
...
HRESULT CMinorous::Release_RenderState(void)
{
...
void CMinorous::Chase(_float fTimeDelta)
{
...
void CMinorous::Chase2(_float fTimeDelta)
{
...
void CMinorous::Chase3(_float fTimeDelta)
{
	_float Distance = D3DXVec3Length(&(*(_float3*)&m_tInfo.pTarget->Get_World().m[3][0] - m_pTransformCom->Get_State(CTransform::STATE_POSITION)));
	if (1.f >= Distance)
	{
...
CMinorous * CMinorous::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CMinorous* pInstance = new CMinorous(_pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CMinorous"));
		Safe_Release(pInstance);
	}
...
CGameObject * CMinorous::Clone(void * pArg)
{
	CMinorous* pInstance = new CMinorous(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CMinorous"));
		Safe_Release(pInstance);
	}
...
_float4x4 CMinorous::Get_World(void)
{
	return m_pTransformCom->Get_WorldMatrix();
}

void CMinorous::Free(void)
{
	__super::Free();
...
HRESULT CMinorous::On_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CMinorous::Off_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CMinorous::Skill_DefaultAttack(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
void CMinorous::Move_Frame(_float fTimeDelta)
{
	switch (m_eCurState)
	{
	case IDLE:
...
void CMinorous::Use_Skill(_float fTimeDelta)
{
...
HRESULT CMinorous::TextureRender()
{
	switch (m_eCurState)
	{
	case IDLE:
...
void CMinorous::MonsterMove(_float fTimeDelta)
{
...
HRESULT CMinorous::RespawnMonster()
{
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, m_tInfo.vPos);
	m_ePreState = STATE_END;
	m_eCurState = IDLE;
