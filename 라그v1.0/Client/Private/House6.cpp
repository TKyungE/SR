#include "stdafx.h"
#include "..\Public\House6.h"
#include "GameInstance.h"


CHouse6::CHouse6(LPDIRECT3DDEVICE9 _pGraphic_Device)
	:CGameObject(_pGraphic_Device)
{
	ZeroMemory(&m_IndexPos, sizeof(INDEXPOS));
}

CHouse6::CHouse6(const CHouse6 & rhs)
	:CGameObject(rhs)
{
}

HRESULT CHouse6::Initialize_Prototype(void)
{
...
HRESULT CHouse6::Initialize(void * pArg)
{
	...
	vPos2.z += 0.375f * vScale.z;
	vPos2.x += 0.255f * vScale.x;
	...
	vPos3.z -= 0.375f * vScale.z;
	vPos3.x += 0.255f * vScale.x;
	...
	m_pTransformCom6->Set_Scaled(vScale2);
	_float3 vPos6 = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	vPos6.y += 0.725f * vScale.y;
	vPos6.x += 0.51f * vScale.x;

	m_pTransformCom6->Turn(_float3(0.f, 1.f, 0.f), 1);
	m_pTransformCom6->Turn(_float3(0.f, 0.f, 1.f), 0.5f);
...
void CHouse6::Tick(_float fTimeDelta)
{
	...
	WorldMatrix.m[0][0] = 10.f;

	WorldMatrix.m[1][1] = 30.f;

	WorldMatrix.m[2][2] = 15.f;
...
void CHouse6::Late_Tick(_float fTimeDelta)
{
...
HRESULT CHouse6::SetUp_Components(void)
{
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_House6"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
HRESULT CHouse6::SetUp_RenderState(void)
{
...
HRESULT CHouse6::Release_RenderState(void)
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CHouse6::RectHouse_Render(void)
{
	if (FAILED(m_pTransformCom->Bind_OnGraphicDev()))
		return E_FAIL;
...
CHouse6 * CHouse6::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CHouse6* pInstance = new CHouse6(_pGraphic_Device);
...
CGameObject * CHouse6::Clone(void * pArg)
{
	CHouse6* pInstance = new CHouse6(*this);
...
_float4x4 CHouse6::Get_World(void)
{
	return m_pTransformCom->Get_WorldMatrix();
}

void CHouse6::Free(void)
{
	__super::Free();
