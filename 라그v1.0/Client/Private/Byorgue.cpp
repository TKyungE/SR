...
CByorgue::CByorgue(const CByorgue& rhs)
	: CGameObject(rhs)
{
}
HRESULT CByorgue::Initialize_Prototype(void)
{
...
HRESULT CByorgue::Initialize(void * pArg)
{
	...
	m_tInfo.iDmg = 66;
	m_tInfo.fX = 0.5f;
	m_tInfo.iMaxHp = 12000;
	m_tInfo.iHp = m_tInfo.iMaxHp;
	m_tInfo.iMp = 1;
	m_tInfo.iExp = 20;
	m_tInfo.iMonsterType = (_int)MON_BYORGUE;
...
void CByorgue::Tick(_float fTimeDelta)
{
...
void CByorgue::Late_Tick(_float fTimeDelta)
{
...
HRESULT CByorgue::SetUp_Components(void)
{
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_IDLE_Front"), (CComponent**)&m_pTextureComIDLE_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_IDLE_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_IDLE_Back"), (CComponent**)&m_pTextureComIDLE_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_Move_Front"), (CComponent**)&m_pTextureComMove_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Move_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_Move_Back"), (CComponent**)&m_pTextureComMove_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_Attack_Front"), (CComponent**)&m_pTextureComAttack_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Attack_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_Attack_Back"), (CComponent**)&m_pTextureComAttack_Back)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Front"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_Dead_Front"), (CComponent**)&m_pTextureComDead_Front)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Texture_Dead_Back"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Byorgue_Dead_Back"), (CComponent**)&m_pTextureComDead_Back)))
		return E_FAIL;
...
HRESULT CByorgue::SetUp_RenderState(void)
{
...
HRESULT CByorgue::Release_RenderState(void)
{
...
void CByorgue::Chase(_float fTimeDelta)
{
...
void CByorgue::Chase2(_float fTimeDelta)
{
...
void CByorgue::Chase3(_float fTimeDelta)
{
	_float Distance = D3DXVec3Length(&(*(_float3*)&m_tInfo.pTarget->Get_World().m[3][0] - m_pTransformCom->Get_State(CTransform::STATE_POSITION)));
	if (1.f >= Distance)
	{
...
CByorgue * CByorgue::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CByorgue* pInstance = new CByorgue(_pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CByorgue"));
		Safe_Release(pInstance);
	}
...
CGameObject * CByorgue::Clone(void * pArg)
{
	CByorgue* pInstance = new CByorgue(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CByorgue"));
		Safe_Release(pInstance);
	}
...
_float4x4 CByorgue::Get_World(void)
{
	return m_pTransformCom->Get_WorldMatrix();
}

void CByorgue::Free(void)
{
	__super::Free();
...
HRESULT CByorgue::On_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CByorgue::Off_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CByorgue::Skill_DefaultAttack(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Sword"), LEVEL_GAMEPLAY, pLayerTag, &tInfo)))
		return E_FAIL;
	CSoundMgr::Get_Instance()->PlayEffect(L"Byorgue_Attack.wav", fSOUND);
	Safe_Release(pGameInstance);

	return S_OK;
}
void CByorgue::Motion_Change()
{
...
void CByorgue::Move_Frame(_float fTimeDelta)
{
	switch (m_eCurState)
	{
	case IDLE:
...
void CByorgue::Check_Front()
{
	...
	if (m_tInfo.bDead && m_eCurState != DEAD)
	{
		...
		CSoundMgr::Get_Instance()->PlayEffect(L"Byorgue_Die.wav", fSOUND);
	}
...
void CByorgue::Use_Skill(_float fTimeDelta)
{
...
HRESULT CByorgue::TextureRender()
{
	switch (m_eCurState)
	{
	case IDLE:
...
void CByorgue::MonsterMove(_float fTimeDelta)
{
...
HRESULT CByorgue::RespawnMonster()
{
	m_pTransformCom->Set_State(CTransform::STATE_POSITION, m_tInfo.vPos);
	m_ePreState = STATE_END;
	m_eCurState = IDLE;
