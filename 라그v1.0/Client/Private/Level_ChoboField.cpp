#include "stdafx.h"
#include "..\Public\Level_ChoboField.h"
...
HRESULT CLevel_ChoboField::Initialize()
{
	...
	CSoundMgr::Get_Instance()->PlayBGM(L"CHOBO.wav", fSOUND + 0.1f);


	return S_OK;
}

void CLevel_ChoboField::Tick(_float fTimeDelta)
{
...
void CLevel_ChoboField::Late_Tick(_float fTimeDelta)
{
...
HRESULT CLevel_ChoboField::Ready_Layer_Player(const _tchar * pLayerTag)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChoboField::Ready_Layer_Monster(const _tchar * pLayerTag)
{
	CGameInstance* pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChoboField::Ready_Layer_Camera(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
HRESULT CLevel_ChoboField::Ready_Layer_UI(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Fade"), LEVEL_CHOBOFIELD, pLayerTag, &tInfo)))
		return E_FAIL;
...
HRESULT CLevel_ChoboField::Ready_Layer_Portal(const _tchar * pLayerTag)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
...
_float3 CLevel_ChoboField::Get_CollisionPos(CGameObject * pDest, CGameObject * pSour)
{
...
void CLevel_ChoboField::Create_Rain(_float fTimeDelta)
{
	CGameInstance*			pGameInstance = CGameInstance::Get_Instance();
	Safe_AddRef(pGameInstance);
		...
		for (auto& iter : pGameInstance->Find_Layer(LEVEL_CHOBOFIELD, TEXT("Layer_Portal"))->Get_Objects())
		{
			if (dynamic_cast<CPortal*>(iter)->Get_Level())
			{
				if (!m_bFade)
				{
					...
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Fade"), LEVEL_CHOBOFIELD, TEXT("Layer_Fade"), &tInfo)))
						return;
					m_bFade = true;
				}

				if (pGameInstance->Find_Layer(LEVEL_CHOBOFIELD, TEXT("Layer_Fade"))->Get_Objects().front()->Get_Info().iMaxMp == 1)
				{
...
{
	CLevel_ChoboField*	pInstance = new CLevel_ChoboField(pGraphic_Device);
