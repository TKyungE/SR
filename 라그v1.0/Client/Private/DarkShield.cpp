#include "stdafx.h"
#include "..\Public\DarkShield.h"
#include "GameInstance.h"
#include "Layer.h"

CDarkShield::CDarkShield(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

CDarkShield::CDarkShield(const CDarkShield & rhs)
	: CGameObject(rhs)
{
}

HRESULT CDarkShield::Initialize_Prototype()
{
...
HRESULT CDarkShield::Initialize(void* pArg)
{
...
void CDarkShield::Tick(_float fTimeDelta)
{
	...
	{
		ERR_MSG(TEXT("Failed to Add CollisionGroup : CDarkShield"));
		return;
	}
	Safe_Release(pInstance);
}

void CDarkShield::Late_Tick(_float fTimeDelta)
{
...
void CDarkShield::Motion_Change()
{
...
void CDarkShield::Move_Frame(_float fTimeDelta)
{
	switch (m_eCurState)
	{
	case IDLE:
...
HRESULT CDarkShield::TextureRender()
{
	switch (m_eCurState)
	{
	case IDLE:
...
HRESULT CDarkShield::On_SamplerState()
{
...
HRESULT CDarkShield::Off_SamplerState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CDarkShield::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CDarkShield::SetUp_RenderState()
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CDarkShield::Release_RenderState()
{
...
CDarkShield * CDarkShield::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CDarkShield*	pInstance = new CDarkShield(pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CDarkShield"));
		Safe_Release(pInstance);
	}
...
CGameObject * CDarkShield::Clone(void* pArg)
{
	CDarkShield*	pInstance = new CDarkShield(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CDarkShield"));
		Safe_Release(pInstance);
	}
...
_float4x4 CDarkShield::Get_World(void)
{
	return m_pTransformCom->Get_WorldMatrix();
}

void CDarkShield::Free()
{
	__super::Free();
