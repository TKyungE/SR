#include "stdafx.h"
#include "..\Public\House7.h"
#include "GameInstance.h"


CHouse7::CHouse7(LPDIRECT3DDEVICE9 _pGraphic_Device)
	:CGameObject(_pGraphic_Device)
{
	ZeroMemory(&m_IndexPos, sizeof(INDEXPOS));
}

CHouse7::CHouse7(const CHouse7 & rhs)
	:CGameObject(rhs)
{
}

HRESULT CHouse7::Initialize_Prototype(void)
{
...
HRESULT CHouse7::Initialize(void * pArg)
{
	...
	vPos4.z -= 0.5f * vScale.z;
	vPos4.x += 0.5f * vScale.x;
	...
	vPos5.z -= 0.5f * vScale.z;
	vPos5.x += 1.5f * vScale.x;
	...
	_float3 vScale2 = _float3(4.05f, 2.3f, 2.f);

	m_pTransformCom7->Set_Scaled(vScale2);
	_float3 vPos7 = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	vPos7.z +=0.5f * vScale.z;
	vPos7.x	+= 1.f * vScale.x;
	vPos7.y += 0.8f * vScale.y;

	m_pTransformCom7->Turn(_float3(1.f, 0.f, 0.f), -0.5f);
	...
	vPos8.z -= 0.5f * vScale.z;
	vPos8.x += 1.f * vScale.x;
	vPos8.y += 0.8f * vScale.y;

	m_pTransformCom8->Turn(_float3(1.f, 0.f, 0.f), 0.5f);
	...
	_float3 vScale3 = _float3(2.f, 1.6f, 2.f);
	
	m_pTransformCom9->Set_Scaled(vScale3);
	_float3 vPos9 = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	vPos9.y += 0.8f * vScale.y;
	vPos9.x += 0.001f;
	m_pTransformCom9->Turn(_float3(0.f, 1.f, 0.f), 1);
	m_pTransformCom9->Set_State(CTransform::STATE_POSITION, vPos9);


	////============================================================================================================

	m_pTransformCom10->Set_Scaled(vScale3);
	_float3 vPos10 = m_pTransformCom->Get_State(CTransform::STATE_POSITION);
	vPos10.x += 2.f * vScale.x;
	vPos10.y += 0.8f * vScale.y;;

	m_pTransformCom10->Turn(_float3(0.f, 1.f, 0.f), 1);
	m_pTransformCom10->Set_State(CTransform::STATE_POSITION, vPos10);
...
void CHouse7::Tick(_float fTimeDelta)
{
	...
	WorldMatrix.m[0][0] = 3.5f;

	WorldMatrix.m[1][1] = 5.f;
...
void CHouse7::Late_Tick(_float fTimeDelta)
{
...
HRESULT CHouse7::Render(void)
{
	...
	if (FAILED(m_pTransformCom9->Bind_OnGraphicDev()))
		return E_FAIL;

	if (FAILED(m_pTextureCom->Bind_OnGraphicDev(3)))
		return E_FAIL;
	m_pVIBufferCom9->Render();



	if (FAILED(m_pTransformCom10->Bind_OnGraphicDev()))
		return E_FAIL;

	if (FAILED(m_pTextureCom->Bind_OnGraphicDev(3)))
		return E_FAIL;
	m_pVIBufferCom10->Render();
...
HRESULT CHouse7::SetUp_Components(void)
{
	...
	if (FAILED(__super::Add_Components(TEXT("Com_VIBuffer7"), LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect2"), (CComponent**)&m_pVIBufferCom8)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_VIBuffer8"), LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect2"), (CComponent**)&m_pVIBufferCom9)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_VIBuffer9"), LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_Rect2"), (CComponent**)&m_pVIBufferCom10)))
		return E_FAIL;

	if (FAILED(__super::Add_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_House7"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
	...
	if (FAILED(__super::Add_Components(TEXT("Com_Transform7"), LEVEL_STATIC, TEXT("Prototype_Component_Transform"), (CComponent**)&m_pTransformCom8, &TransformDesc)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Transform8"), LEVEL_STATIC, TEXT("Prototype_Component_Transform"), (CComponent**)&m_pTransformCom9, &TransformDesc)))
		return E_FAIL;
	if (FAILED(__super::Add_Components(TEXT("Com_Transform9"), LEVEL_STATIC, TEXT("Prototype_Component_Transform"), (CComponent**)&m_pTransformCom10, &TransformDesc)))
		return E_FAIL;
...
HRESULT CHouse7::SetUp_RenderState(void)
{
...
HRESULT CHouse7::Release_RenderState(void)
{
	if (nullptr == m_pGraphic_Device)
		return E_FAIL;
...
HRESULT CHouse7::RectHouse_Render(void)
{
	if (FAILED(m_pTransformCom->Bind_OnGraphicDev()))
		return E_FAIL;
...
CHouse7 * CHouse7::Create(LPDIRECT3DDEVICE9 _pGraphic_Device)
{
	CHouse7* pInstance = new CHouse7(_pGraphic_Device);
...
CGameObject * CHouse7::Clone(void * pArg)
{
	CHouse7* pInstance = new CHouse7(*this);
...
_float4x4 CHouse7::Get_World(void)
{
	return m_pTransformCom->Get_WorldMatrix();
}

void CHouse7::Free(void)
{
	__super::Free();
	...
	Safe_Release(m_pTransformCom9);
	Safe_Release(m_pTransformCom10);
	...
	Safe_Release(m_pVIBufferCom9);
	Safe_Release(m_pVIBufferCom10);
