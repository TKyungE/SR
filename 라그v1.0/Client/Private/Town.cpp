...
HRESULT CTown::Ready_Layer_BackGround(const _tchar * pLayerTag)
{
	...
	for (auto& iter : m_vecTree)
	{
		...
		indexpos.iLevelIndex = LEVEL_TOWN;
		...
		for (auto& iter : pGameInstance->Find_Layer(LEVEL_TOWN, TEXT("Layer_Portal"))->Get_Objects())
		{
			if (dynamic_cast<CPortal*>(iter)->Get_Level())
			{
				if (!m_bFade)
				{
					...
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Fade"), LEVEL_TOWN, TEXT("Layer_Fade"), &tInfo)))
						return;
					m_bFade = true;
				}

				if (pGameInstance->Find_Layer(LEVEL_TOWN, TEXT("Layer_Fade"))->Get_Objects().front()->Get_Info().iMaxMp == 1)
				{
